            ...
            <div class="col-md-12">
                ...
                <form method="POST" action="{{ route('events.store') }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="name" name="name" value="@if(isset($event)){{ $event->name }}@else{{ old('name') }}@endif">
                    </div>
                    <div class="form-group">
                        <label for="description">{{__('Description of event')}}</label>
                        <textarea class="form-control" rows=5 id="description" name="description">@if(isset($event)){{ $event->description }}@else{{ old('description') }}@endif</textarea>
                    </div>
                    <div class="form-group">
                        <label for="start_date">{{__('Start Date of event')}}</label>
                        <input type="date" class="form-control" id="start_date" name="start_date" value="@if(isset($event)){{ old('start_date') }} @else{{ old('start_date') }}@endif">
                    </div>
                    <div class="form-group">
                        <label for="end_date">{{__('End Date of event')}}</label>
                        <input type="date" class="form-control" id="end_date" name="end_date" value="@if(isset($event)){{ $event->end_date }}@else{{ old('end_date') }}@endif">
                    </div>
                    <div class="form-group">
                        <label for="status">{{__('Status of event')}}  %</label>
                        <input type="number" class="form-control" id="status" name="status" value="{{ $event->status ?? 0 }}">
                    </div>
                    <div class="form-group">
                        <label for="control_date">{{__('Control Date of event')}}</label>
                        <input type="date" class="form-control" id="control_date" name="control_date" value="@if(isset($event)){{ $event->control_date }}@else{{ old('control_date') }}@endif">
                    </div>
                    @php
                        ...
                        <select name="projects_id[]" id="projects_id" class="form-control" multiple>
                            @foreach($projects as $project)
                                <option value="{{ $project->id }}"
                                    @if(isset($event) && $event->projects->contains($project->id)) selected @endif
                                    >{{ $project->name }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <select name="steps_id[]" id="steps_id" class="form-control" multiple>
                            @foreach($steps as $step)
                                <option value="{{ $step->id }}"
                                    @if(isset($event) && $event->steps->contains($step->id)) selected @endif
                                    >{{ $step->name }}</option>
                            @endforeach
                        </select>
                    </div>
