            ...
            <div class="col-md-12">
                <form method="POST" action="{{ route('stages.update',$stage) }}">
                    ...
                    <div class="form-group mb-2">
                        ...
                        <label>{{__('Stages')}}</label>
                        <select name="stages_id[]" class="form-control" multiple>
                            @foreach($stages as $stage)
                                <option value="{{ $stage->id }}" @if(in_array($stage->id, $stage->nextstages->pluck('id')->toArray())) selected @endif>{{ $stage->name }}</option>
                            @endforeach
                        </select>
                    </div>
